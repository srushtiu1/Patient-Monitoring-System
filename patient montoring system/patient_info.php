...
while($row = mysqli_fetch_array($result)){
    ...
    $a="select COUNT(1) from payment where pm_b_no=(select max(b_no) from bill where b_p_id='$p_id')";
    $result = mysqli_query($link,$a);
		...
		if($row['COUNT(1)']>0)
		{
		$qry="select * from payment where pm_b_no=(select max(b_no) from bill where b_p_id='$p_id')";
    $result = mysqli_query($link,$qry);
		while($row = mysqli_fetch_array($result))
		{
      
      if($row['excess']==0 && $row['pending']==0)
      {
        $b_bal=mysqli_real_escape_string($link,$_POST['b_amt']);
...
if(mysqli_query($link, $sql)){
    ...
    echo "ERROR: Could not able to execute $sql. " . mysqli_error($link);
      ...
      else if($row['excess']!=0 && $row['pending']==0)
      {
        $b=mysqli_real_escape_string($link,$_POST['b_amt']);
        $b_bal=$b-$row['excess'];
      
   $sql = "INSERT INTO bill (b_p_id, b_date,b_amt,b_bal) VALUES ('$p_id', '$b_date', '$b_amt','$b_bal')";
if(mysqli_query($link, $sql)){
	$q="update payment set excess=0 where pm_b_no=(SELECT b_no FROM bill where b_p_id='$p_id' ORDER BY b_no DESC LIMIT 1,1 )";
  if(mysqli_query($link, $q)){
	$p="SELECT b_no FROM bill where b_p_id='$p_id' ORDER BY b_no DESC LIMIT 1,1";
    $ans=mysqli_query($link, $p);
    while($row=mysqli_fetch_array($ans))
    {
    $z=$row['b_no'];
  $sql1="UPDATE bill set b_bal=0 where  b_no='$z' ";
 if(mysqli_query($link, $sql1)){
   ...
   $sql = "INSERT INTO bill (b_p_id, b_date,b_amt,b_bal) VALUES ('$p_id', '$b_date', '$b_amt','$b_bal')";
if(mysqli_query($link, $sql)){
	$q="update payment set pending=0 where pm_b_no=(SELECT b_no FROM bill where b_p_id='$p_id' ORDER BY b_no DESC LIMIT 1,1 )";
  if(mysqli_query($link, $q)){
    $p="SELECT b_no FROM bill where b_p_id='$p_id' ORDER BY b_no DESC LIMIT 1,1";
    $ans=mysqli_query($link, $p);
    while($row=mysqli_fetch_array($ans))
...
$s="update visits set billed='y' where v_p_id='$p_id'";
  if(mysqli_query($link, $s)){
    ...
    {
      $b=mysqli_real_escape_string($link,$_POST['b_amt']);
      $qry="select b_bal,max(b_no) from bill where b_p_id=$p_id";
      $result=mysqli_query($link, $qry);
    while($row=mysqli_fetch_array($result))
    {
      $b_bal=$b+$row['b_bal'];
